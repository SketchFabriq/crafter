...
class BaseLocobotEnv(RobotTaskEnv):
    OBS_DIM = 19
    ACTION_DIM = 4
    GOAL_DIM = 3
...
class LocobotTouchEnv(BaseLocobotEnv):
    def __init__(self, render_mode="rgb_array", control_type="ee", **kwargs):
        ...
        BaseLocobotEnv.GOAL_DIM = task.get_goal().shape[0]


class LocobotGraspEnv(BaseLocobotEnv):
    def __init__(self, render_mode="rgb_array", control_type="ee", **kwargs):
        sim = PyBullet(render_mode=render_mode, renderer="Tiny")
        robot = Locobot(
            sim,
...
class LocobotLiftEnv(BaseLocobotEnv):
    def __init__(self, render_mode="rgb_array", control_type="ee", **kwargs):
        sim = PyBullet(
...
class LocobotPickPlaceEnv(BaseLocobotEnv):
    def __init__(self, render_mode="rgb_array", control_type="ee", **kwargs):
        sim = PyBullet(render_mode=render_mode, renderer="Tiny")
        robot = Locobot(
            sim,
